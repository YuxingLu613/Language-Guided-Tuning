...
"""Train Housing price regression with LLM–driven tabular data augmentation.

Usage:
    python scripts/train_housing_augmented.py --config config/housing_config.yaml

The script is almost identical to `train_housing.py` but instantiates
`TabularAugTrainer` instead of `HousingTrainer` and therefore enables the
augmentation chooser when `augmentation.enabled=true` in the YAML config.
"""
...
def main(config_path: Path, model_path: Optional[Path] = None):
    ...
    log_dir = (ROOT_DIR / config["logging"]["log_dir"]).joinpath(f"housing_aug_{ts}")
    log_dir.mkdir(parents=True, exist_ok=True)
    config["logging"]["log_dir"] = str(log_dir)

    # Save used config and use this path for trainer
...
if __name__ == "__main__":
    ...
    ap.add_argument("--config", default="config/housing_config.yaml")
