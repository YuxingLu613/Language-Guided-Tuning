...
def load_water_quality(cfg: Dict[str, Any], seed: int) -> Tuple[DataLoader, DataLoader]:
    """读取水质数据并划分训练/验证集。"""
    ...
    return DataLoader(train_ds, shuffle=True, **dl_kwargs), DataLoader(val_ds, shuffle=False, **dl_kwargs)
...
def main(config_path: Path, model_path: Optional[Path] = None):
    ...
    new_log_dir = f"{orig_log_dir}_water_baseline_{ts}"
    ...
    # 数据加载
    train_loader, val_loader = load_water_quality(cfg, seed)
    ...
    if model_cls is None:
        raise RuntimeError(f"未在 {mod} 找到 nn.Module 子类以构建模型")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Water Quality Baseline 训练脚本 (无超参数调优)")
    ap.add_argument("--config", default="config/water_quality_config.yaml", help="配置文件路径")
