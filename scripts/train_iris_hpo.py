...
def main(cfg_path: Path, model_path: Optional[Path] = None):
    ...
    cfg["logging"]["log_dir"] += f"_iris_hpo_{ts}"
    ...
    trainer = Trainer(model, tmp_cfg)  # 保留 LLM Advisor 进行 HPO
    print("开始 Iris HPO 训练，日志目录:", cfg["logging"]["log_dir"])
    trainer.train(train_loader, val_loader)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Iris HPO Training (LLM Advisor)")
    parser.add_argument("--config", default="config/iris_config.yaml", help="配置文件路径")
