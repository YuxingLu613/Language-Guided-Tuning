...
class Trainer:
    def __init__(self, model: Optional[nn.Module], config_path: Union[str, Path, dict]):
        """
        支持 model=None 的初始化（用于策略建议场景，无需实际to(device)）。
        """
        ...
        # 优化器仅在有 model 时创建，用于 dummy (Agent建议)时允许返回 None。
        self.optimizer = self._init_optimizer() if self.model is not None else None
    ...
    def _init_optimizer(self) -> Optional[torch.optim.Optimizer]:
        """优化器仅在模型存在时初始化，用于策略建议可直接 skip"""
    ...
    def train_epoch(self, train_loader: DataLoader, epoch: Optional[int] = None, is_evaluation: bool = False) -> Dict[str, float]:
        ...
        """
        if self.model is None:
            raise RuntimeError("Model is not initialized. Cannot train.")
    ...
    def validate(self, val_loader: DataLoader) -> Dict[str, float]:
        ...
        """
        if self.model is None:
            raise RuntimeError("Model is not initialized. Cannot validate.")
    ...
    def _cache_baseline_results(self, train_loader: DataLoader, val_loader: Optional[DataLoader] = None) -> Dict[str, Any]:
        ...
        """
        if self.model is None:
            raise RuntimeError("Model is not initialized. Cannot cache baseline results.")
    ...
    def _evaluate_optimization(self,
        ...
        if self.model is None:
            raise RuntimeError("Model is not initialized. Cannot perform optimization.")
