...
class StrategyAgent:
    ...
    def _build_prompt(
        ...
        # Try to get from current_strategy first, fallback to config training section
        current_loss = current_strategy.get("loss_function")
        current_opt = current_strategy.get("optimizer")
        
        # If not in current_strategy, try to get from config
        if current_loss is None:
            current_loss = context_cfg.get("training", {}).get("loss_function", "N/A")
        if current_opt is None:
            current_opt = context_cfg.get("training", {}).get("optimizer", "N/A")
            
        prompt += "Current loss function: " + str(current_loss) + "\n"
        prompt += "Current optimizer: " + str(current_opt) + "\n\n"
    ...
    def decide_strategy(
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": self._build_prompt(task_type, current_strategy, metrics, context_cfg, guidance, previous_context),
            },
        ]
