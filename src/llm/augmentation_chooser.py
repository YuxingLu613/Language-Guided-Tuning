...
class LLMAugmentationChooser:
    ...
    _SYSTEM = (
        "You are a computer-vision model-training expert. Given the current training/validation "
    ...
    def _build_prompt(self, metrics: Dict[str, Any], context: Dict[str, Any], guidance: str | None = None) -> str:
        """Compose prompt with metrics and extra context for better decision."""
        import json
        prompt = "Available methods: " + ", ".join(self.available_augs) + "\n\n"
    ...
    def choose(self, metrics: Dict[str, Any], context: Dict[str, Any], guidance: str | None = None) -> Tuple[str, bool]:
        headers = {
        ...
        messages = [
            ...
            {"role": "user", "content": self._build_prompt(metrics, context, guidance)},
        ]
        data = {
            "model": "deepseek-chat",
            "messages": messages,
